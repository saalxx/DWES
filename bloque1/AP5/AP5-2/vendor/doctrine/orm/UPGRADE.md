...
## BC BREAK: Calling
`ClassMetadata::getAssociationMappedByTargetField()` with the owning side of an association now throws an exception
...
* If you want to upgrade your existing tables to identity columns, you will need to
  follow [migration to identity columns on PostgreSQL](https://www.doctrine-project.org/projects/doctrine-dbal/en/4.0/how-to/postgresql-identity-migration.html)
...
## BC BREAK: Changed `EntityManagerInterface#refresh($entity)`, `EntityManagerDecorator#refresh($entity)` and
`UnitOfWork#refresh($entity)` signatures
...
## BC BREAK: Omitting only the alias argument for `QueryBuilder::update` and
`QueryBuilder::delete` is not supported anymore
...
* Instantiate caches yourself: to use a different cache implementation, pass a cache instance when calling any
  ...
  ```
* As a quick workaround, you can lock the doctrine/cache dependency to work around this:
...
deprecated in favor of calling AbstractQuery#getQueryCacheProfile(). This method returns a
...
The allowPartialObjects configuration option together with the `Configuration#getAllowPartialObjects` and
`Configuration#setAllowPartialObjects` methods have been removed.
